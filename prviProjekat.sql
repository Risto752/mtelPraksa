...
FROM
    covid_vacinations
    where 
    location like '%Croatia%'
ORDER BY
    TO_NUMBER(people_fully_vaccinated) desc;
...
-- trazimo zemlje sa najvecim brojem vakcinisanih po glavi stanovnika
    
    SELECT
   dea.location,
   population,
    MAX(TO_NUMBER(people_fully_vaccinated)) AS broj_vakcinisanih,
     MAX(TO_NUMBER(people_fully_vaccinated) / population) * 100 AS percent_population_vaccinated
FROM
    covid_deaths dea
    JOIN covid_vacinations vac ON dea.location = vac.location
                                   AND dea.dan = vac.dan
where dea.location like '%Bosnia%'
GROUP BY
    dea.location,
    population
